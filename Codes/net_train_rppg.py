...
iD_ir = path_dir + subjects[1]+im_mode[2]
...
#%% load Mat file
x = loadmat(path_dir + subjects[1]+'/PulseOX/pulseOx.mat')
...
rv = [randint(0,6230) for _ in range(5000)]
randint(0,5350)
rv =  np.array(rv)
...
# Training parameters.
learning_rate = 0.0001 # start with 0.001
...
in3 = neural_net.layers[9](in2).numpy()
# well we can go on
...
neural_net.layers[5].set_weights(Basenet.layers[5].get_weights())